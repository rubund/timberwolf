 ...
 */
#ifndef lint
static char SccsId[] = "@(#) include.c (Yale) version 1.2 8/24/89" ; 
#endif
	...
	&& (st.st_mode & S_IFMT) == S_IFLNK) {
		*pp++ = copy(buf);
		if (pp >= &notdotdot[ MAXDIRS ]){
			fprintf( stderr, "out of .. dirs, increase MAXDIRS\n");
			exit(1) ;
		}
		return(TRUE);
	}
	...
	register char	*newfile, *incstring;
{
	...
	if (inclistp == inclist + MAXFILES - 1){
		fprintf( stderr, "out of space: increase MAXFILES\n");
		exit(1) ;
	}
	...
	if (ip->i_list == NULL)
		...
		for (i=0; i<ip->i_listlen; i++)
			if (ip->i_list[ i ] == newfile) {
				...
				fprintf(stderr,"%s includes %s more than once!\n",
					ip->i_file, newfile->i_file);
				fprintf(stderr,"Already have\n");
				for (i=0; i<ip->i_listlen; i++)
					fprintf(stderr,"\t%s\n", ip->i_list[i]->i_file);
			    }
			    return;
