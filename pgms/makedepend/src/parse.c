 ...
 */
#ifndef lint
static char SccsId[] = "@(#) parse.c (Yale) version 1.2 8/24/89" ; 
#endif
	...
	int			recursion;
{
	...
	while (line = get_line(filep)) {
		...
		case UNDEF:
			 ...
			 */
			if (!*line) {
			    fprintf( stderr,"%s, line %d: incomplete undef == \"%s\"\n",
		...
		case -1:
			...
			if (file_red != file)
			    fprintf( stderr," (reading %s)", file->i_file);
		...
		case -2:
			...
			if (file_red != file)
			    fprintf( stderr," (reading %s)", file->i_file);
			fprintf( stderr,", line %d: incomplete include == \"%s\"\n",
			    filep->f_line, line);
			break;
		}
	}
