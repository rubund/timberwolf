...
----------------------------------------------------------------- */
#ifndef lint
static char SccsId[] = "@(#) readnets.y (Yale) version 1.5 2/21/91" ;
#endif
...
{
    if( netS ){
/* routing not concerned in tomus*/
    }
} /* end ignore_route */
...
char    *s;
{
    sprintf(YmsgG,"problem reading %s.net in Tomus:", cktNameG );
    M( ERRMSG, "yacc", YmsgG ) ;
...
add_paths_to_cells()
{
    ...
    total_cells = numcellsG + numtermsG ;
