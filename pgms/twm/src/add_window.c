...
Window w;
{
    ...
    if( tmp_win->attr.width + tmp_win->attr.border_width > MyDisplayWidth ){
	D( fprintf( stderr, "Resized width from %d ", tmp_win->attr.width ) ) ;
	tmp_win->attr.width = MyDisplayWidth - tmp_win->attr.border_width ;
	D( fprintf( stderr, "to %d\n", tmp_win->attr.width ) ) ;

    }
    if( tmp_win->attr.height + tmp_win->title_height > MyDisplayHeight ){
	D( fprintf( stderr, "Resized height from %d ", tmp_win->attr.height ) ) ;
	tmp_win->attr.height = MyDisplayHeight - 
	    tmp_win->title_height - 2 * BorderWidth ; 
	D( fprintf( stderr, "to %d\n", tmp_win->attr.height ) ) ;
    }
    ...
    {
#ifdef sun
	XSetWindowBorder(dpy, tmp_win->frame, BorderTileC.fore);
	XSetWindowBorder(dpy, tmp_win->title_w, BorderTileC.fore);
#else
