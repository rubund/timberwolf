...
/********************************************************************
 *
 *  move_right places all nets and connects all pins to the right of
 ...
 *  Date: 4-8-84
 *
 *  Modified 3-26-85 by D. Braun. See left.c for details.
 *
 *******************************************************************/

move_right(channel, cost, top, bottom, initial_column,
		num_rows, num_cols, num_nets)
...
IN int initial_column;		/* column to move right from */
...
IN int num_nets;		/* just here for place_all_nets */
{
    ...
    for (column = initial_column + 1; column <= num_cols; column++)
    {
	/********
